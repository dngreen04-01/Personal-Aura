...
export const ACTION_TIMER_CONTINUE = 'timer-continue';
  ...
  if (Platform.OS === 'ios') {
    await notifee.setNotificationCategories([
      ...
      },
      {
        id: 'block-timer',
        actions: [
          { id: ACTION_TIMER_CONTINUE, title: 'Continue' },
          { id: ACTION_EXTEND_15S, title: '+15s' },
        ],
      },
    ]);
 ...
 */
export async function fireAlarmNow(restId, { playSound = true } = {}) {
  try {
    ...
    // Stop foreground service and dismiss the persistent timer notification
    if (Platform.OS === 'android') {
...
// --- Block Timer Notifications (interval/amrap/emom) ---

const BLOCK_TIMER_NOTIF_ID = 'block-timer-countdown';
const BLOCK_ALARM_NOTIF_ID = 'block-timer-alarm';

/**
 * Show a persistent countdown notification for a block timer.
 * Dynamic title and body based on phase and label.
 */
export async function showBlockTimerNotification(label, totalSeconds, timerId, phase) {
  try {
    const mins = Math.floor(totalSeconds / 60);
    const secs = totalSeconds % 60;
    const timeStr = `${mins}:${secs.toString().padStart(2, '0')}`;
    const title = phase === 'rest' ? 'Rest Phase' : 'Work Phase';
    const endTimestamp = Date.now() + totalSeconds * 1000;

    await notifee.displayNotification({
      id: BLOCK_TIMER_NOTIF_ID,
      title,
      body: `${label} \u2014 ${timeStr} remaining`,
      data: { timerId: String(timerId || '') },
      android: {
        channelId: 'rest-timer', // reuse existing low-importance channel
        asForegroundService: true,
        ongoing: true,
        autoCancel: false,
        showChronometer: true,
...
/**
 * Fire the block timer alarm immediately (phase or block complete).
 */
export async function fireBlockAlarm(label, timerId) {
  try {
    if (Platform.OS === 'android') {
      await notifee.stopForegroundService().catch(() => {});
    }
    await notifee.cancelNotification(BLOCK_TIMER_NOTIF_ID).catch(() => {});

    await notifee.displayNotification({
      id: BLOCK_ALARM_NOTIF_ID,
      title: 'Phase Complete!',
      body: label || 'Ready to continue',
      data: { timerId: String(timerId || '') },
      android: {
        channelId: 'rest-alarm',
        pressAction: { id: 'default' },
        actions: [
          { title: 'Continue', pressAction: { id: ACTION_TIMER_CONTINUE } },
          { title: '+15s', pressAction: { id: ACTION_EXTEND_15S } },
        ],
        color: '#d4ff00',
        smallIcon: 'ic_notification',
...
/**
 * Dismiss the block timer notification (called on phase transition).
 */
export async function dismissBlockTimerNotification() {
  try {
    if (Platform.OS === 'android') {
      await notifee.stopForegroundService().catch(() => {});
    }
    await notifee.cancelNotification(BLOCK_TIMER_NOTIF_ID).catch(() => {});
    await notifee.cancelNotification(BLOCK_ALARM_NOTIF_ID).catch(() => {});
  } catch {}
}
