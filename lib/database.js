...
export async function getTrainingContext(days = 7) {
  ...
  for (const row of rows) {
    ...
    // Parse muscle groups from exercises_cache
    const primaryMuscles = row.primary_muscles ? JSON.parse(row.primary_muscles) : [];
    const secondaryMuscles = row.secondary_muscles ? JSON.parse(row.secondary_muscles) : [];
    ...
    // Track per-exercise history (use most recent set data)
