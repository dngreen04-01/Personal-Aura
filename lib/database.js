...
async function initializeDatabase(database) {
  ...
  if (!sessionCols.map(c => c.name).includes('location_id')) {
...
// Workout Sessions
export async function startSession(planDay, focus, locationId = null) {
  const database = await getDatabase();
  const result = await database.runAsync(
    'INSERT INTO workout_sessions (plan_day, focus, location_id) VALUES (?, ?, ?)',
    [planDay, focus, locationId]
  );
...
export async function setExerciseUnitPreference(exerciseName, unit) {
  const database = await getDatabase();
  await database.runAsync(
...
// Locations
export async function saveLocation(name, equipmentList, isDefault = false) {
  const database = await getDatabase();
  const result = await database.runAsync(
    'INSERT INTO locations (name, equipment_list, is_default) VALUES (?, ?, ?)',
    [name, JSON.stringify(equipmentList), isDefault ? 1 : 0]
  );
  return result.lastInsertRowId;
}
...
export async function updateLocation(id, name, equipmentList) {
  const database = await getDatabase();
  await database.runAsync(
    "UPDATE locations SET name = ?, equipment_list = ?, updated_at = datetime('now') WHERE id = ?",
    [name, JSON.stringify(equipmentList), id]
  );
}
...
export async function setDefaultLocation(id) {
  const database = await getDatabase();
  await database.runAsync('UPDATE locations SET is_default = 0');
  await database.runAsync('UPDATE locations SET is_default = 1 WHERE id = ?', [id]);
}
...
export async function getPersonalRecords() {
  ...
  return await database.getAllAsync(`
    ...
    second_best AS (
      ...
      GROUP BY LOWER(exercise_name)
      HAVING COUNT(DISTINCT ROUND(CASE WHEN weight_unit = 'lbs' THEN weight * 0.453592 ELSE weight END, 1)) > 1
    )
    SELECT
      b.exercise_name,
      b.pr_weight,
...
// Progressive Overload: Get exercise progression data (goal-aware, unit-aware)
export async function getExerciseProgressionData(exerciseName, weeks = 4, preferredUnit = 'kg') {
  ...
  // Normalize all historical weights to kg for consistent comparison
  const rows = await database.getAllAsync(`
    SELECT
  ...
  // Plateau: no weight increase in last 6+ sets (compare rounded to avoid float drift)
  ...
  // Goal-aware progression thresholds (increments in kg, converted later)
  ...
  // Progressive overload logic (all in kg)
  ...
  if (avgRpe !== null) {
    if (avgRpe < pushThreshold) {
      ...
      // Convert increment to display unit for the reason text
  ...
  // Convert suggested weight to preferred unit for display
...
// Get maximum weight ever lifted for an exercise (normalized to kg)
...
/// Greeting: Collect data needed for the pre-workout greeting
export async function getGreetingData() {
  const db = await getDatabase();
  const [streak, sessionCount, latestSession] = await Promise.all([
    getWorkoutStreak(),
    getCompletedSessionCount(),
    db.getFirstAsync('SELECT plan_day, focus, started_at FROM workout_sessions ORDER BY started_at DESC LIMIT 1'),
  ]);
  return {
    streak,
    sessionCount,
