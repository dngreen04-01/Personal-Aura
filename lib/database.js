...
import { getDefaultIncrement } from './weightUtils';
...
async function initializeDatabase(database) {
  ...
  if (!sessionColNames.includes('exercises_json')) {
    await database.execAsync('ALTER TABLE workout_sessions ADD COLUMN exercises_json TEXT');
  }
  if (!sessionColNames.includes('position_json')) {
    await database.execAsync('ALTER TABLE workout_sessions ADD COLUMN position_json TEXT');
  }
  ...
  // === Phase 0: Block-based architecture (additive, non-breaking) ===
  // See Docs/AI-Driven Fitness App Refactor.md §3. Legacy workout_sets stays
  // authoritative through Phase 1. These tables are created alongside and
  // will be populated once the Planning Agent emits block plans.
  await database.execAsync(`
    CREATE TABLE IF NOT EXISTS session_blocks (
      ...
      block_type TEXT NOT NULL CHECK (block_type IN (
        'strength','interval','amrap','emom','circuit','timed','distance','cardio','rest'
      )),
      label TEXT,
      config_json TEXT,
      created_at TEXT DEFAULT (datetime('now')),
    ...
    CREATE TABLE IF NOT EXISTS block_entries (
      ...
      entry_type TEXT NOT NULL CHECK (entry_type IN (
        'strength_set','timed_effort','distance_effort','round','rest'
      )),
      payload_json TEXT,
      logged_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT,
  ...
  // user_profile: additive JSON columns for new canonical taxonomy
  const profileColNames2 = (await database.getAllAsync("PRAGMA table_info(user_profile)")).map(c => c.name);
  if (!profileColNames2.includes('goals_json')) {
    await database.execAsync('ALTER TABLE user_profile ADD COLUMN goals_json TEXT');
  }
  if (!profileColNames2.includes('style_preferences_json')) {
    await database.execAsync('ALTER TABLE user_profile ADD COLUMN style_preferences_json TEXT');
  }
  if (!profileColNames2.includes('sport_context_json')) {
    await database.execAsync('ALTER TABLE user_profile ADD COLUMN sport_context_json TEXT');
  }
  if (!profileColNames2.includes('injuries_json')) {
    await database.execAsync('ALTER TABLE user_profile ADD COLUMN injuries_json TEXT');
  }

  // workout_sessions: immutable plan snapshot (dual-source elimination per PRD).
  // Legacy exercises_json remains readable for rollback safety for >=2 versions.
  const sessionColNames2 = (await database.getAllAsync("PRAGMA table_info(workout_sessions)")).map(c => c.name);
  if (!sessionColNames2.includes('plan_snapshot_json')) {
    await database.execAsync('ALTER TABLE workout_sessions ADD COLUMN plan_snapshot_json TEXT');
  }

  // active_rest_timer → generalized into an active_timer surface.
  // Keep the table name for rollback safety; add additive columns so the
  // same row can describe rest, interval, amrap, emom countdowns, etc.
  const timerColNames = (await database.getAllAsync("PRAGMA table_info(active_rest_timer)")).map(c => c.name);
  if (!timerColNames.includes('timer_kind')) {
    await database.execAsync("ALTER TABLE active_rest_timer ADD COLUMN timer_kind TEXT DEFAULT 'rest'");
  }
  if (!timerColNames.includes('context_json')) {
    await database.execAsync('ALTER TABLE active_rest_timer ADD COLUMN context_json TEXT');
  }
  if (!timerColNames.includes('alarm_fired_at')) {
    await database.execAsync('ALTER TABLE active_rest_timer ADD COLUMN alarm_fired_at INTEGER');
  }
}
...
export async function saveUserProfile(goal, equipment, experience, bodyStats = {}, schedule = {}, { syncToCloud = true, goalsJson = null, stylePreferencesJson = null, sportContextJson = null, injuriesJson = null } = {}) {
  const database = await getDatabase();
  ...
  // Populate Phase 4 JSON columns if provided
  if (goalsJson || stylePreferencesJson || sportContextJson || injuriesJson) {
    await database.runAsync(
      'UPDATE user_profile SET goals_json = ?, style_preferences_json = ?, sport_context_json = ?, injuries_json = ? WHERE id = (SELECT MAX(id) FROM user_profile)',
      [
        goalsJson ? JSON.stringify(goalsJson) : null,
        stylePreferencesJson ? JSON.stringify(stylePreferencesJson) : null,
        sportContextJson ? JSON.stringify(sportContextJson) : null,
        injuriesJson ? JSON.stringify(injuriesJson) : null,
      ]
    );
  }
  ...
  if (syncToCloud) {
    ...
    if (goalsJson) cloudPayload.goals_json = goalsJson;
    if (stylePreferencesJson) cloudPayload.style_preferences_json = stylePreferencesJson;
    if (sportContextJson) cloudPayload.sport_context_json = sportContextJson;
    if (injuriesJson) cloudPayload.injuries_json = injuriesJson;
    pushToCloud('profile', null, cloudPayload, 'set');
  }
}
...
export async function startSession(planDay, focus, locationId = null, { syncToCloud = true, exercisesJson = null } = {}) {
  const database = await getDatabase();
  const now = new Date().toISOString();
  let firestoreId = null;
  ...
  const result = await database.runAsync(
    'INSERT INTO workout_sessions (plan_day, focus, location_id, exercises_json, updated_at, firestore_id) VALUES (?, ?, ?, ?, ?, ?)',
    [planDay, focus, locationId, exercisesJson, now, firestoreId]
  );
...
export async function saveSessionState(sessionId, day, positionState) {
  const database = await getDatabase();
  await database.runAsync(
    'UPDATE workout_sessions SET exercises_json = ?, position_json = ?, updated_at = ? WHERE id = ?',
    [JSON.stringify(day), JSON.stringify(positionState), new Date().toISOString(), sessionId]
  );
}
...
export async function getIncompleteSession() {
  ...
  return database.getFirstAsync(
    `SELECT * FROM workout_sessions
     WHERE ended_at IS NULL AND exercises_json IS NOT NULL
...
export async function discardSession(sessionId) {
  const database = await getDatabase();
  await database.runAsync(
    "UPDATE workout_sessions SET ended_at = datetime('now'), duration_seconds = 0, updated_at = ? WHERE id = ?",
...
export async function getSessionState(sessionId) {
  const database = await getDatabase();
  return database.getFirstAsync(
    'SELECT exercises_json, position_json FROM workout_sessions WHERE id = ?',
    [sessionId]
  );
}
...
// --- Session Blocks & Block Entries ---

export async function createSessionBlock(sessionId, blockIndex, blockType, label, configJson) {
  const database = await getDatabase();
  const result = await database.runAsync(
    `INSERT INTO session_blocks (session_id, block_index, block_type, label, config_json)
     VALUES (?, ?, ?, ?, ?)`,
    [sessionId, blockIndex, blockType, label || '', typeof configJson === 'string' ? configJson : JSON.stringify(configJson || {})]
  );
  return result.lastInsertRowId;
}

export async function logBlockEntry(blockId, entryIndex, entryType, payloadJson) {
  const database = await getDatabase();
  const result = await database.runAsync(
    `INSERT INTO block_entries (block_id, entry_index, entry_type, payload_json)
     VALUES (?, ?, ?, ?)`,
    [blockId, entryIndex, entryType, typeof payloadJson === 'string' ? payloadJson : JSON.stringify(payloadJson || {})]
  );
  return result.lastInsertRowId;
}
...
export async function getSessionBlocks(sessionId) {
  const database = await getDatabase();
  return await database.getAllAsync(
    'SELECT * FROM session_blocks WHERE session_id = ? ORDER BY block_index',
    [sessionId]
  );
}

export async function getBlockEntries(blockId) {
  const database = await getDatabase();
  return await database.getAllAsync(
    'SELECT * FROM block_entries WHERE block_id = ? ORDER BY entry_index',
    [blockId]
  );
}

export async function createBlocksFromPlan(sessionId, validatedPlan) {
  const blockIds = [];
  for (const block of validatedPlan.blocks) {
    const id = await createSessionBlock(
      sessionId,
      block.block_index,
      block.block_type,
      block.label,
    ...
    blockIds.push({ blockIndex: block.block_index, blockId: id, label: block.label, blockType: block.block_type });
  }
  return blockIds;
}

export async function logStrengthSet(sessionId, blockId, exerciseName, setNumber, weight, weightUnit, reps, rpe, restSeconds, opts) {
  const setId = await logSet(sessionId, exerciseName, setNumber, weight, weightUnit, reps, rpe, restSeconds, opts);
  if (blockId) {
    try {
      await logBlockEntry(blockId, setNumber, 'strength_set', {
        exercise: exerciseName, weight, weight_unit: weightUnit, reps, rpe, rest_seconds: restSeconds
      });
    } catch (_) { /* block write is fire-and-forget; legacy is authoritative */ }
  }
  return setId;
}
...
// isLowerBodyExercise moved to weightUtils.js as LOWER_BODY_KEYWORDS
...
export async function getExerciseProgressionData(exerciseName, weeks = 4, preferredUnit = 'kg') {
  ...
  const equipmentIncrement = getDefaultIncrement('kg', exerciseName);

  let pushThreshold, deloadThreshold, incrementKg;
  if (goal.includes('strength')) {
    pushThreshold = 8;
    deloadThreshold = 9.5;
    incrementKg = equipmentIncrement;
  } else if (goal.includes('fat') || goal.includes('lose')) {
    // Fat loss: progress less frequently, not in smaller jumps. Half-plate
    // increments don't exist on a barbell — a real coach programs the same
    // plate jumps, just rarer.
    pushThreshold = 5;
    deloadThreshold = 8.5;
    incrementKg = equipmentIncrement;
  } else {
...
export async function getGreetingData() {
  const database = await getDatabase();
  const [streakObj, sessionCount, latestSession, weekCountRow] = await Promise.all([
    getWorkoutStreak(),
    getCompletedSessionCount(),
    database.getFirstAsync('SELECT plan_day, focus, started_at FROM workout_sessions ORDER BY started_at DESC LIMIT 1'),
    // Sessions completed since this week's Monday (local time). `weekday 0` rolls forward to the next Sunday;
    // subtracting 6 days lands on the most recent Monday.
    database.getFirstAsync(`
      SELECT COUNT(*) as count
      FROM workout_sessions
      WHERE ended_at IS NOT NULL
        AND date(started_at, 'localtime') >= date('now', 'localtime', 'weekday 0', '-6 days')
    `),
  ]);
  return {
    streak: streakObj?.current ?? 0,
    streakLongest: streakObj?.longest ?? 0,
    sessionCount,
    lastWorkoutFocus: latestSession?.focus || null,
    lastWorkoutDate: latestSession?.started_at || null,
    weekCompleted: weekCountRow?.count ?? 0,
  };
}
...
export async function markRestTimerAlarmFired(firedAt = Date.now()) {
  const database = await getDatabase();
  await database.runAsync(
    'UPDATE active_rest_timer SET alarm_fired_at = ? WHERE id = 1',
    [firedAt]
  );
}

// --- Block Timer (generalized timer persistence for interval/amrap/emom) ---

export async function saveBlockTimer(endTime, sessionId, timerKind, contextJson) {
  const database = await getDatabase();
  await database.runAsync(
    `INSERT OR REPLACE INTO active_rest_timer (id, rest_end_time, session_id, timer_kind, context_json)
     VALUES (1, ?, ?, ?, ?)`,
    [endTime, sessionId, timerKind, typeof contextJson === 'string' ? contextJson : JSON.stringify(contextJson || {})]
  );
}

export async function getActiveBlockTimer() {
  const database = await getDatabase();
  const row = await database.getFirstAsync('SELECT * FROM active_rest_timer WHERE id = 1');
  if (row && row.context_json) {
    try { row.context = JSON.parse(row.context_json); } catch (_) { row.context = {}; }
  }
  return row;
}

// --- Entry Logging Helpers (timer/round blocks — no dual-write needed) ---

export async function logTimedEffort(blockId, entryIndex, elapsedSec, contextJson) {
  return logBlockEntry(blockId, entryIndex, 'timed_effort', {
    elapsed_sec: elapsedSec,
    ...(contextJson || {}),
  });
}

export async function logRoundEntry(blockId, entryIndex, roundNum, movementsCompleted) {
  return logBlockEntry(blockId, entryIndex, 'round', {
    round: roundNum,
    movements_completed: movementsCompleted || [],
  });
}

export async function logDistanceEffort(blockId, entryIndex, distanceM, elapsedSec, contextJson) {
  return logBlockEntry(blockId, entryIndex, 'distance_effort', {
...
export async function logRestEntry(blockId, entryIndex, durationSec) {
  return logBlockEntry(blockId, entryIndex, 'rest', {
