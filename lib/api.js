...
export async function greetUser(userContext, { locationName, locationsCount } = {}) {
  const controller = new AbortController();
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/api/agent/greet`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ userContext, locationName, locationsCount }),
      signal: controller.signal,
    });
...
export async function sendElicitationMessage(message, history = []) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 30000);

  try {
    const headers = await authHeaders();
    const res = await fetch(`${BASE_URL}/api/onboarding/elicit`, {
      method: 'POST',
      headers,
    ...
    if (!res.ok) {
      const error = await res.json();
      throw new Error(error.error || 'Elicitation failed');
    }
...
export async function generateMultiModalityPlan(profile) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 90000);

  try {
    const headers = await authHeaders();
    const res = await fetch(`${BASE_URL}/api/onboarding/generate`, {
      method: 'POST',
      headers,
