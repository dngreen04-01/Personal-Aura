...
async function initialSyncIfNeeded(uid) {
  ...
  // 1. Profile
  const profileData = await getDocData(userDocRef(uid, 'profile'));
  if (profileData) {
...
export function pushToCloud(collection, documentId, data, operation) {
  ...
  // Queue locally first (non-blocking)
  queueSync(collection, documentId, operation, { ...data, _uid: currentUid }).catch(err =>
    console.warn('[Sync] Failed to queue sync item:', err.message)
  );

  // Attempt immediate write (async, non-blocking)
  attemptFirestoreWrite(currentUid, collection, documentId, data, operation).catch(() => {
    // Failed — will retry from queue later
  });
}

async function attemptFirestoreWrite(uid, collectionPath, documentId, data, operation) {
  ...
  // If immediate write succeeds, remove from queue
  const database = await getDatabase();
  const pending = await database.getFirstAsync(
    "SELECT id FROM sync_queue WHERE collection = ? AND document_id = ? AND status = 'pending' ORDER BY created_at DESC LIMIT 1",
    [collectionPath, documentId]
  );
  if (pending) {
    await markSyncComplete(pending.id);
  }
}

function buildFirestoreRef(uid, collectionPath, documentId) {
  if (collectionPath === 'profile') {
    return userDocRef(uid, 'profile');
  }
...
function setupRealtimeListeners(uid) {
  // Profile listener
  const profileRef = userDocRef(uid, 'profile');
